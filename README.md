# Conversor de notação para jogos de xadrez  
O programa deve aceitar as jogadas dum jogo de xadrez, em sequência, numa das seguintes notações:  

1. [Algébrica](https://pt.wikipedia.org/wiki/Nota%C3%A7%C3%A3o_alg%C3%A9brica_de_xadrez)
1. [Descritiva](https://pt.wikipedia.org/wiki/Nota%C3%A7%C3%A3o_descritiva_de_xadrez) ( usar a descrição em Inglês dado que a em Português está off...)
1. [Postal](https://pt.wikipedia.org/wiki/Nota%C3%A7%C3%A3o_epistolar)  
...
- As ações "algebrica", "descritiva" e "postal" devem apresentar um registo do jogo, à medida que vai sendo jogado.
...
2. Vários ficheiros de input, com indicação duma (ou mais) linha de comando para processar cada um.
3. Um relatório breve, que:  
    1. Explique as opções tomadas (representação das jogadas, representação do tabuleiro, como é que se codificam as jogadas permitidas, etc.)
    1. Explique a organização do código
    1. Indique os pontos fortes e fraquezas do programa
    1. Discuta quais poderiam ser as evoluções caso tivesse mais 1 mês para dedicar ao trabalho
